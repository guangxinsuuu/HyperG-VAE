...
parser.add_argument('--batch_size', type=int, default=50, help='The batch size used in the training process.')
...
parser.add_argument('--seed', type=int, default=2023, help='seed, default =1')
...
parser.add_argument('--dataset_name', type=str, default='STRING500hESC')

#-------------------------------------------------
if parser.parse_args().dataset_name == "STRING500hESC":
    parser.add_argument('--data_file', type=str, default='demo_data\GRN_inference\inputdata\STRING_500_hESC\data.csv',help='The input scRNA-seq gene expression file.')
    parser.add_argument('--net_file', type=str, default='demo_data\GRN_inference\inputdata\STRING_500_hESC\label.csv',
                    help='The ground truth of GRN. Only used in GRN inference task if available. ')

if parser.parse_args().dataset_name == "STRING1000hESC":
    parser.add_argument('--data_file', type=str, default='demo_data\GRN_inference\inputdata\STRING_1000_hESC\data.csv',help='The input scRNA-seq gene expression file.')
    parser.add_argument('--net_file', type=str, default='demo_data\GRN_inference\inputdata\STRING_1000_hESC\label.csv',
                    help='The ground truth of GRN. Only used in GRN inference task if available. ')


opt = parser.parse_args()
opt.alpha = 0.1*10**(opt.nalpha)
opt.beta =  1*10**(-opt.nbeta)
opt.omega = 0.1*opt.nomega
opt.lr = 1*10**(-opt.nlr)
opt.dropout = 0.1*opt.dropoutrate
opt.lr_step_size = 1*10**(-opt.nlr_step_size)
seed = opt.seed
opt.save_name = opt.dataset_name
